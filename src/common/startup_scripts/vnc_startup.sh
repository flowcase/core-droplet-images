...
function pull_profile (){
	if [ ! -z "$KASM_PROFILE_LDR" ]; then
		...
		set +e
		if [[ $DEBUG == true ]]; then
			OUTPUT=$(http_proxy="" https_proxy="" /usr/bin/kasm-profile-sync --download /home/flowcase-user --insecure --remote ${KASM_API_HOST} --port ${KASM_API_PORT} -c ${KASM_PROFILE_CHUNK_SIZE} --token ${KASM_API_JWT} --verbose 2>&1 )
		else
			OUTPUT=$(http_proxy="" https_proxy="" /usr/bin/kasm-profile-sync --download /home/flowcase-user --insecure --remote ${KASM_API_HOST} --port ${KASM_API_PORT} -c ${KASM_PROFILE_CHUNK_SIZE} --token ${KASM_API_JWT} 2>&1 )
		fi
...
function start_vnc (){
	...
	FLOWCASE_PROCS['kasmvnc']=$(cat $HOME/.vnc/*${DISPLAY_NUM}.pid)
	...
	if [[ $DEBUG == true ]]; then
	  echo -e "\n------------------ Started Websockify  ----------------------------"
	  echo "Websockify PID: ${FLOWCASE_PROCS['kasmvnc']}";
	fi
}
...
function start_audio_out_websocket (){
	if [[ ${KASM_SVC_AUDIO:-1} == 1 ]]; then
		...
		$STARTUPDIR/jsmpeg/flowcase_audio_out_linux flowcaseaudio 8081 4901 ${HOME}/.vnc/self.pem ${HOME}/.vnc/self.pem "flowcase_user:$VNC_PW"  &

		FLOWCASE_PROCS['flowcase_audio_out_websocket']=$!

		if [[ $DEBUG == true ]]; then
		  echo -e "\n------------------ Started Audio Out Websocket  ----------------------------"
		  echo "Flowcase Audio Out Websocket PID: ${FLOWCASE_PROCS['flowcase_audio_out_websocket']}";
		fi
	fi
}
...
function start_audio_out (){
	if [[ ${KASM_SVC_AUDIO:-1} == 1 ]]; then
		...
		if [[ $DEBUG == true ]]; then
			...
			HOME=/var/run/pulse no_proxy=127.0.0.1 ffmpeg -f pulse -fragment_size ${PULSEAUDIO_FRAGMENT_SIZE:-2000} -ar 44100 -i default -f mpegts -correct_ts_overflow 0 -codec:a mp2 -b:a 128k -ac 1 -muxdelay 0.001 http://127.0.0.1:8081/flowcaseaudio &
			FLOWCASE_PROCS['flowcase_audio_out']=$!
		else
			echo 'Starting audio service'
			HOME=/var/run/pulse no_proxy=127.0.0.1 ffmpeg -v verbose -f pulse -fragment_size ${PULSEAUDIO_FRAGMENT_SIZE:-2000} -ar 44100 -i default -f mpegts -correct_ts_overflow 0 -codec:a mp2 -b:a 128k -ac 1 -muxdelay 0.001 http://127.0.0.1:8081/flowcaseaudio > /dev/null 2>&1 &
			FLOWCASE_PROCS['flowcase_audio_out']=$!
			echo -e "\n------------------ Started Audio Out  ----------------------------"
			echo "Flowcase Audio Out PID: ${FLOWCASE_PROCS['flowcase_audio_out']}";
		fi
	fi
}
...
function start_audio_in (){
	if [[ ${KASM_SVC_AUDIO_INPUT:-1} == 1 ]]; then
		...
		FLOWCASE_PROCS['kasm_audio_in']=$!

		if [[ $DEBUG == true ]]; then
			echo -e "\n------------------ Started Audio Out Websocket  ----------------------------"
			echo "Kasm Audio In PID: ${FLOWCASE_PROCS['kasm_audio_in']}";
		fi
	fi
}
...
function start_upload (){
	if [[ ${KASM_SVC_UPLOADS:-1} == 1 ]]; then
		...
		if [[ $DEBUG == true ]]; then
			...
			echo "Upload Server PID: ${FLOWCASE_PROCS['upload_server']}";
		fi
	fi
}
...
function start_gamepad (){
	if [[ ${KASM_SVC_GAMEPAD:-1} == 1 ]]; then
		...
		FLOWCASE_PROCS['kasm_gamepad']=$!

		if [[ $DEBUG == true ]]; then
			echo -e "\n------------------ Started Gamepad Websocket  ----------------------------"
			echo "Kasm Gamepad PID: ${FLOWCASE_PROCS['kasm_gamepad']}";
		fi
	fi
}
...
function start_webcam (){
	if [[ ${KASM_SVC_WEBCAM:-1} == 1 ]] && [[ -e /dev/video0 ]]; then
		...
		FLOWCASE_PROCS['kasm_webcam']=$!

		if [[ $DEBUG == true ]]; then
			echo -e "\n------------------ Started Webcam Websocket  ----------------------------"
			echo "Kasm Webcam PID: ${FLOWCASE_PROCS['kasm_webcam']}";
		fi
	fi
}
...
function start_printer (){
		...
		FLOWCASE_PROCS['kasm_printer']=$!

		if [[ $DEBUG == true ]]; then
			echo -e "\n------------------ Started Printer Service  ----------------------------"
			echo "Kasm Printer PID: ${FLOWCASE_PROCS['kasm_printer']}";
		fi
	fi
}
...
echo -e "${VNC_PW}\n${VNC_PW}\n" | kasmvncpasswd -u flowcase_user -wo
...
KASMIP=$(hostname -i)
log "Flowcase User ${KASM_USER}(${KASM_USER_ID}) started container id ${HOSTNAME} with local IP address ${KASMIP}" "INFO"
...
while :
do
	for process in "${!FLOWCASE_PROCS[@]}"; do
		if ! kill -0 "${FLOWCASE_PROCS[$process]}" ; then
